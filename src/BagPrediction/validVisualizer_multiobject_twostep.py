...
if __name__ == '__main__':
    ...
    representation = GraphRepresentation.GraphRepresentation_rigid_deformable(SimulatedData.keypoint_indices,
    ...
    model_path_mask = "./models/test-11"
    ...
    for i_scenario in range(10):
        ...
        for i_frame in range(newdata.num_frames):
            ...
            if i_frame == 0:
                ...
                current_predict_tuples = compute_output(module=module_dyn, inputs_tr=prev_input_graph_tuples)
                ...
                # replace the cloth node position for rendering, the first L nodes are cloth nodes
                data_vis.dataset_cloth[i_scenario][i_frame][
                        representation.keypoint_indices] = copy.copy(current_node_recover[:len(representation.keypoint_indices), :3])

                # prepare the next input
                # substract the current effector pose to transform the current predicted frame to new local coordinate
                current_node_recover[:,:3] -= gt_current_effector_pose[:3]
                # renew the 4th and 5th attributes
                current_node_recover[:,3:] = prev_graph_dict['nodes'][:,3:]

                current_node_align = current_node_recover
                current_receivers_pos = np.float32(current_node_align[current_predict_tuples[-1].receivers.numpy()])
                current_senders_pos = np.float32(current_node_align[current_predict_tuples[-1].senders.numpy()])
                ...
                newedge[:,3:6] = 0
                newedge[representation.combineindices_1, 3:6] = newedge[representation.combineindices_1,
                                                        :3]  # denote the physical connection
                newedge[representation.combineindices_2, 3:6] = newedge[representation.combineindices_2,
                                                        :3]  # denote the physical connection
